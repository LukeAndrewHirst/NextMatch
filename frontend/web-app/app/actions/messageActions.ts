...
export async function getMessageThread(recipientId: string) {
    try {
        ...
        const messages = await prisma.message.findMany({
            ...
            orderBy: {created: 'asc'},select: messageSelect
        })
        ...
        if(messages.length > 0) {
            ...
            await prisma.message.updateMany({where: {id: {in: readMessageIds}}, data: {dateRead: new Date()}})
...
export async function getMessagesByContainer(container: string) {
    try {
        ...
        const messages = await prisma.message.findMany({where: conditions, orderBy: {created: 'desc'}, select: messageSelect})
...
export async function deleteMessage(messageId: string, isOutbox: boolean) {
    ...
    try {
        ...
        await prisma.message.update({where: {id: messageId},data: {[selector]: true}})
...
export async function getUnreadMesageCount() {
    try{
        const userId = await getAuthUserId(); 

        return prisma.message.count({where: { recipientId: userId, dateRead: null, recipientDeleted: false}})
    } catch(error) {
        console.log(error);
        throw error;
    }
